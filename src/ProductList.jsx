    ...
    useEffect(()=>{
        if (cartItems.length===0){
            ...
            cartItems.forEach((product, idx)=>{
                if (idx===0) {
                    setAddedToCart({[product.name]: true})
                } else{
                    setAddedToCart((prev)=>({...prev, [product.name]: true}))
                }
            })
        ...
        <div>
            <div className="navbar" style={styleObj}>
                ...
                <div style={styleObjUl}>
                    ...
                    <div style={{position: "relative"}} onClick={(e) => handleCartClick(e)}>
                        ...
                        {!!totalQuantity &&  <span className='cart_quantity_count'>{totalQuantity}</span>}
                    </div>
                </div>
            </div>
            {!showCart ? (
                <div className="product-grid">
                    {!!plantsArray.length && plantsArray.map((elem, rootIdx)=>(
                        <React.Fragment key={rootIdx}>
                            <h2>{elem.category}</h2>
                            {!!elem.plants.length && (
                                <div className='product-list'>
                                    {elem.plants.map((plant, plantIdx)=>(
                                        <div className="product-card" key={`${elem.category}-${plantIdx}`}>
                                            <img src={plant.image} alt={plant.name} className='product-image'/>
                                            <div className='product-content'>
                                                <h4 className="product-title">{plant.name}</h4>
                                                <div className='product-description'>{plant.description}</div>
                                                <div className='product-price'>{plant.cost}</div>
                                                <button type="button" className={`product-button ${addedToCart[plant.name]? "added-to-cart": ''}`}
                                                onClick={() => handleAddToCart(plant)}>{addedToCart[plant.name]? "Added ✔":"Add to Cart"}</button>
                                            </div>
                                        </div>
                                    ))}
                                </div>
